...
'*********************
'Thomas Swift
Declare Function RegDeleteValue Lib "advapi32.dll" Alias "RegDeleteValueA" (ByVal hkey As Long, ByVal Value As String) As Long
'*********************
...
'*********************
'Thomas Swift
Function DeleteValue(SubKey As String, ByVal Value As String)
Call ParseKey(SubKey, MainKeyHandle)
If MainKeyHandle Then
   rtn = RegOpenKeyEx(MainKeyHandle, SubKey, 0, KEY_WRITE, hkey)
   If rtn = ERROR_SUCCESS Then
    rtn = RegDeleteValue(hkey, Value)
...
If MainKeyHandle Then
   ...
   If rtn = ERROR_SUCCESS Then 'if the key could be opened then
      ...
      If rtn = ERROR_SUCCESS Then 'if the value could be retreived then
         ...
         GetStringValue = TrimNull(Left(sBuffer, Len(sBuffer) - 1)) 'return the value to the user
      Else                        'otherwise, if the value couldnt be retreived
...
'****************************************************************************
Public Function EnumRegistryKeys(ByVal SubKey As String) As Collection
    Dim handle As Long
    Dim length As Long
    Dim index As Long
    ...
    Dim fFiletime As FILETIME
         ...
         If Len(SubKey) Then
             If RegOpenKeyEx(MainKeyHandle, SubKey, 0, KEY_READ, handle) Then Exit Function
             ' in all case the subsequent functions use hKey
             MainKeyHandle = handle
         End If
         ...
         Do
             ...
             If RegEnumKeyEx(MainKeyHandle, index, subkeyName, length, 0, "", vbNull, fFiletime) = ERROR_NO_MORE_ITEMS Then Exit Do
